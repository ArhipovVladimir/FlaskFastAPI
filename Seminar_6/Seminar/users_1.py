...
async def create_note(count: int):
    for i in range(1, count+1):
        query = users_db.insert().values(login=f'user_{i}', password=f'pswd_{i}', email=f'mail_{i}@mail.ru')
...
async def create_user(user: InputUser):
    query = users_db.insert().values(login=user.login, password=user.password, email=user.email)
